...
namespace LibreHardwareMonitor.Hardware.CPU
{
    internal class CpuLoad
    {
        ...
        {
            Interop.Ntdll.SystemProcessorPerformanceInformation[] informations = new Interop.Ntdll.SystemProcessorPerformanceInformation[64];
            int size = Marshal.SizeOf(typeof(Interop.Ntdll.SystemProcessorPerformanceInformation));

            idle = null;
            total = null;

            IntPtr returnLength;
            if (Interop.Ntdll.NtQuerySystemInformation(Interop.Ntdll.SystemInformationClass.SystemProcessorPerformanceInformation, informations, informations.Length * size, out returnLength) != 0)
                return false;
