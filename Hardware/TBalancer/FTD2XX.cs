...
namespace LibreHardwareMonitor.Hardware.TBalancer
{
    ...
    internal class FTD2XX
    {
        ...
        {
            uint bytesWritten;
            FT_STATUS status = FT_Write(handle, buffer, (uint)buffer.Length, out bytesWritten);
        ...
        public static int BytesToRead(FT_HANDLE handle)
        {
            ...
            uint eventStatus;
            if (FT_GetStatus(handle, out amountInRxQueue, out amountInTxQueue, out eventStatus) == FT_STATUS.FT_OK)
        ...
        {
            uint bytesReturned;
            FT_STATUS status = FT_Read(handle, buffer, (uint)buffer.Length, out bytesReturned);
