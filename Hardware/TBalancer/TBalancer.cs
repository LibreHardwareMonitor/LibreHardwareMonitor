...
namespace LibreHardwareMonitor.Hardware.TBalancer
{
    internal class TBalancer : Hardware
    {
            ...
            base("T-Balancer bigNG", new Identifier("bigng", portIndex.ToString(CultureInfo.InvariantCulture)), settings)
        {
            ...
            {
                _sensorhubFlows[i] = new Sensor("Flowmeter " + (i + 1), i, SensorType.Flow, this, new[] {
                    new ParameterDescription("Impulse Rate", "The impulse rate of the flowmeter in pulses/L", 509) }, settings);
            }
        ...
        private void ReadData()
        {
            ...
            { // bigNG
                ...
                for (int i = 0; i < _fans.Length; i++)
                {
                    ...
                    if (_fans[i] == null)
                    {
                        _fans[i] = new Sensor("Fan Channel " + i, i, SensorType.Fan, this, new[] { new ParameterDescription("MaxRPM",
        ...
        public void Open()
        {
            ...
            FTD2XX.FT_SetFlowControl(_handle, FT_FLOW_CONTROL.FT_FLOW_RTS_CTS, 0x11, 0x13);
