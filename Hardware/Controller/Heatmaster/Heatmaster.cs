...
namespace LibreHardwareMonitor.Hardware.Controller
{
    internal class Heatmaster : Hardware, IDisposable
    {
        ...
        {
            return WriteField(device, field, value.ToString(CultureInfo.InvariantCulture));
        }
        ...
        public Heatmaster(string portName, ISettings settings) : base("Heatmaster", new Identifier("heatmaster", portName.TrimStart(new[] { '/' }).ToLowerInvariant()), settings)
        {
            _portName = portName;
            try
            {
                ...
                for (int i = 0; i < fanCount; i++)
                {
                    ...
                    _controls[i] = new Sensor(name, device, SensorType.Control, this, settings);
